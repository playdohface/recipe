...
"#;
        ...
        let set_cherry = Command::SetDirective(SetDirective {
            variable: SelectionPath(vec![Selection::Key("topping")]),
            value: Val::Literal(json!("cherry")),
        });
        let echo_topping = Command::CodeBlock(CodeBlock {
            executor: Some("sh"),
            name: None,
            type_hint: None,
            annotations: vec![],
...
"#;
        ...
        assert_eq!(
            tokens.map(|t| t.inner().clone()).collect_vec(),
            vec![
